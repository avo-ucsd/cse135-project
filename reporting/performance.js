...
function initReportActions() {
    ...
    async function findReportPkByReportId(reportId) {
        const q = new URLSearchParams({ report_id: reportId, page: 'performance', category: 'Performance', limit: '1' });
        const res = await apiFetch(`/reports?${q.toString()}`);
        return Number(res?.data?.[0]?.id ?? 0);
    }

    saveBtn.addEventListener('click', async () => {
        const name = window.prompt('Report name:', `Performance Report — ${new Date().toLocaleDateString('en-US')}`);
        if (!name) return;
        ...
        try {
            ...
            console.error('[performance:reports] create failed', err);
    ...
    fileInput.addEventListener('change', async () => {
        ...
        try {
            ...
            console.error('[performance:reports] upload failed', err);
...
function initAnalystNotes() {
    ...
    async function loadNote() {
        try {
            ...
            console.error('[performance:notes] load failed', err);
    ...
    notes.addEventListener('input', () => {
        ...
        saveTimer = window.setTimeout(async () => {
            ...
            try {
                ...
                console.error('[performance:notes] save failed', err);
...
function initComments() {
    ...
    async function refreshComments() {
        try {
            ...
            console.error('[performance:comments] load failed', err);
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            console.error('[performance:comments] post failed', err);
    ...
    list.addEventListener('click', async (e) => {
        ...
        if (action === 'delete') {
            ...
            try {
                ...
                console.error('[performance:comments] delete failed', err);
        ...
        if (action === 'save-edit') {
            ...
            try {
                ...
                console.error('[performance:comments] edit failed', err);
...
async function init() {
    ...
    // Initialize analyst workflows independent of analytics fetch success.
    initReportActions();
    initAnalystNotes();
    initComments();
    maybeAutoPrint();
